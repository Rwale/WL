...
export default function Home() {
  ...
  function exportExcel() { const params=new URLSearchParams(); if(week)params.set("week",week);if(campaign)params.set("activation",campaign);if(location)params.set("location",location);if(status)params.set("status",status);window.location.href="/api/export?"+params.toString(); }
...
function ReviewPanel(props:{rows:AnyRow[];data:Snapshot;onView:(r:AnyRow)=>void;onReview:(r:AnyRow)=>void}) {
  return <section className="panel table-panel">
    <div className="panel-title"><div><h2>Review queue</h2><p>Approve reports individually, then export only that outlet and week into the master workbook.</p></div></div>
    <div className="sheet-wrap"><table><thead><tr><th>REPORT</th><th>OUTLET</th><th>SUBMITTED BY</th><th>STATUS</th><th>LAST REVIEW</th><th>ACTION</th></tr></thead>
      <tbody>{props.rows.length?props.rows.map(r=>{
        ...
        const canExport=props.data.permissions.canReview&&(r.status==="Approved"||r.status==="Completed");
        return <tr key={r.id}><td>Week {r.week} - {r.activationDate}</td><td>{r.outletName}</td><td>{props.data.users.find(u=>u.id===r.submittedBy)?.name}</td><td>{r.status}</td><td>{review?review.newStatus+": "+review.comment:"Not reviewed"}</td><td><div className="row-actions"><button onClick={()=>props.onView(r)}>Open</button><button disabled={!props.data.permissions.canReview} onClick={()=>props.onReview(r)}>Review</button>{canExport&&<button onClick={()=>{window.location.href=`/api/export-report?id=${r.id}`}}>Export Excel</button>}</div></td></tr>;
