...
export async function POST(request:Request) {
  ...
  const blob=await put(key,bytes,{access:"public",addRandomSuffix:false,contentType:excelType});
  try {
...
export async function PUT(request:Request){
  ...
  const object=await fetch(item.objectKey);if(!object.ok)return error("Stored workbook not found.",404);
...
export async function DELETE(request:Request) {
  ...
  const [existing] = await ctx.db.select().from(workbookTemplates).where(eq(workbookTemplates.id,id)).limit(1);
  if (!existing) return error("Workbook template not found.", 404);
  if (process.env.BLOB_READ_WRITE_TOKEN) await del(existing.objectKey);
...
export async function GET(request:Request) {
  ...
  const object = await fetch(item.objectKey);
  if (!object.ok) return new Response("Stored workbook not found", { status:404 });
